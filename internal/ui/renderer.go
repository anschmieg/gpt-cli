...
type Renderer struct {
	tty bool
	// glamour is the Glamour term renderer used when tty is true; nil when not available.
...
// Render renders a markdown fragment. If a Glamour renderer is available
// it will be used; otherwise the fragment is returned with minimal
// normalization.
func (r *Renderer) Render(fragment string) string {
	if r == nil || fragment == "" {
		return ""
	}
	if r.glamour != nil {
		if s, err := r.glamour.Render(fragment); err == nil {
