...
func TestStreamSuggestion(t *testing.T) {
	...
	suggestion, err := mode.StreamSuggestion("list files")
	require.NoError(t, err)
	assert.Equal(t, "ls -l", suggestion.Command)
	assert.Equal(t, "safe", suggestion.SafetyLevel)
	assert.Contains(t, suggestion.Explanation, "List files")
