...
// CallProvider makes a request to Copilot API
func (p *CopilotProvider) CallProvider(prompt string) (string, error) {
	if p.config.BaseURL == "" {
		return "", fmt.Errorf("COPILOT_API_BASE not configured")
	}
...
// StreamProvider streams responses from Copilot API
func (p *CopilotProvider) StreamProvider(prompt string) (<-chan string, <-chan error) {
