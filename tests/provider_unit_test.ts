...
Deno.test("chatCompletion returns mocked content without network", async () => {
  const fakeFetch = (_url: string, _init?: RequestInit) => {
    return Promise.resolve(new Response(JSON.stringify({
      choices: [{ message: { content: "pure mock result", role: "assistant" } }],
    }), { status: 200, headers: { "Content-Type": "application/json" } }));
  };

  const out = await chatCompletion({ messages: [{ role: "user", content: "hi" }] }, { fetcher: fakeFetch });
...
Deno.test("chatCompletion surfaces provider errors", async () => {
  ...
  let threw = false;
  try {
    await chatCompletion({ messages: [{ role: "user", content: "hi" }] }, { fetcher: badFetch });
  } catch (e) {
    threw = true;
